...
# VERSION=0 WORKERS=6 MS=qwen34i CONFIG=eval/fea_host.yaml REPO=django HASH=e13b714 eval/fea.sh
...
HASH=${HASH:-e13b714}
VERSION=${VERSION:-1}
CONFIG=${CONFIG:-eval/fea_host.yaml}
WORKERS=${WORKERS:-12}
RUN_EVAL=${RUN_EVAL:-true}

REPO_HASH=${REPO}_${HASH}
OUTPUT_DIR=eval_results/fea/$REPO_HASH/$MS/v$VERSION
RUN_ID=fea_${REPO_HASH}_${MS}_v${VERSION}

uv run mini-extra swebench -c $CONFIG --workers $WORKERS \
    --subset eval/data/fea/$REPO.jsonl \
    --model $MODEL \
